...
class Simulator:
    ...
    # usare un pandas dataframe per riorganizzare il marking e fare fill 0 di tutti i token che compaiono in seguito o scompaiono
    # ora quando il token è zero non viene plottato...
    def make_charts(self, exclude=[]):
        ...
        for net in d:
            for place in d[net]:
                ...
                plt.title(title)
                ymax = 0
                # xrange = arange(self._curr_step + 1)
                # yzeros = zeros(self._curr_step + 1, dtype=int32)

                for i, token in enumerate(d[net][place]):
                    if any(e in token for e in exclude):
                        continue
                    if "_net" not in token:
                        X = d[net][place][token].keys()
                        Y = d[net][place][token].values()
                        # put(yzeros, list(X), list(Y))
                        # ax.plot(xrange, yzeros, label=token.lstrip("_"), color=custom_cmap[token])

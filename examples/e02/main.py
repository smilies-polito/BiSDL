...
class E02(Module):
	...
	def build_net_structure(self) -> PetriNet:
		...
		e02_net = PetriNet("e02_net", timescale=10)
		...
		# e02_net places
		e02_net.add_place(Place("s1", [ p1_0_net ]))
		e02_net.add_place(Place("s2", [ p2_0_net ]))

		# e02_net transitions
		e02_net.add_transition(Transition("juxtacrine_signaling_A_molecule_s1_s2_1", Expression("str(x) == 'A_molecule'")))

		# e02_net arcs
		e02_net.add_input("s1", "juxtacrine_signaling_A_molecule_s1_s2_1", Variable('x'), notify=[p1_0_net])
		e02_net.add_output("s2", "juxtacrine_signaling_A_molecule_s1_s2_1", Expression('x.replace("protein", "receptor_active_protein")'), notify=[p2_0_net])

		# p1_0_net arcs
		p1_0_net.add_input("A_molecule", "process_1", Value(dot), notify=[e02_net.place('s1')])
		p1_0_net.add_output("B_molecule", "process_1", MultiArc([Value(dot)]*3), notify=[e02_net.place('s1')])

		# p2_0_net arcs
		p2_0_net.add_input("B_molecule", "B_molecule_degradation_1", Value(dot), notify=[e02_net.place('s2')])
...
if __name__ == "__main__":
	...
	marking['e02_net']['s1'].add(["A_molecule"]*50)
